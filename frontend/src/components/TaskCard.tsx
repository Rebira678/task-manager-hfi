...
const PRIORITY_STYLES: Record<Task['priority'], { bg: string; text: string; dot: string }> = {
...
export function TaskCard({ task, onUpdate, onDelete }: TaskCardProps) {
  ...
  return (
    <>
      <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm hover:shadow-card transition-all duration-200 group">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2">
          ...
          {task.due_date && (
            <span className={`inline-flex items-center gap-1 text-xs font-medium ${overdue ? 'text-red-600' : 'text-gray-500'}`}>
